...
public class IssueProcessorService {
    ...
    public void process(JsonNode issue, Runnable cleanup) {
        ...
        try {
            ...
            linearClient.postComment(issueId, "❌ Worker 에러가 발생했습니다:\n```\n%s\n```".formatted(e.getMessage()));
        } finally {
            cleanup.run();
    ...
    private void doProcess(JsonNode issue) {
        ...
        if (route == null) {
            linearClient.postComment(issueId, "⚠️ 팀 키 `%s`에 대한 레포 라우팅이 설정되지 않았습니다.".formatted(teamKey));
            return;
        }
        ...
        if (!repoDir.exists()) {
            linearClient.postComment(issueId, "⚠️ 레포 디렉토리가 없습니다: `%s`\n`git clone`으로 먼저 클론해주세요.".formatted(repoDir));
            return;
        }
    ...
    private void handleHighConfidence(
            File worktreeDir, JsonNode issue, RepoRoute route, String branchName, ClaudeCodeRunner.Result result) {
        String issueId = issue.path("id").asText();
        String identifier = issue.path("identifier").asText();
        String title = issue.path("title").asText();
        String description = issue.path("description").asText("");
        ...
        String prBody =
                ...
                **Confidence:** %s | **Priority:** P%d
                ...
                ---
                🤖 Auto-generated by KillBug Worker"""
                        .formatted(
                                identifier,
                                issueUrl,
                                result.confidence(),
                                issue.path("priority").asInt(3),
                                title);

        String prUrl = githubService.createDraftPr(worktreeDir, prTitle, prBody, route.getBaseBranch(), branchName);
