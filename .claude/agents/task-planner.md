...
### 6. 복잡도 평가 및 서브에이전트 분할 (컨텍스트 부패 방지)

계획 수립 후 반드시 복잡도를 평가한다. 컨텍스트가 커지면 Claude의 규칙 준수 품질이 저하되므로 (컨텍스트 부패), 큰 작업은 서브에이전트로 분할하여 각각 독립 컨텍스트에서 실행한다.

**복잡도 기준:**

| 복잡도 | 조건 | 권장 |
|--------|------|------|
| 낮음 | Phase 3개 이하, 파일 5개 이하 | 단일 컨텍스트에서 순차 실행 |
| 높음 | Phase 4개 이상 또는 파일 6개 이상 | 서브에이전트 분할 권장 |
| 매우 높음 | 2개 이상 모듈 동시 수정 | 서브에이전트 분할 필수 |

**분할 가능 기준 (독립성):**
- 서로 다른 모듈 수정 (webhook-server vs worker) → 분할 가능
- 서로 다른 계층에서 독립적 (Entity/Repository vs Controller/DTO) → 분할 가능
- 같은 파일을 수정하거나 의존 관계가 있으면 → 분할 불가

**분할 시 출력 형식:**

복잡도가 높음 이상이면 플랜 마지막에 아래 섹션을 추가한다:

```
### 서브에이전트 분할 권장

**복잡도**: 높음 (Phase {N}개, 파일 {M}개)
**분할 이유**: {이유}

| 에이전트 | 담당 Phase | 담당 파일 | 의존성 |
|---------|-----------|----------|--------|
| Agent 1 | Phase 1-2 | Entity, Repository | 없음 (먼저 실행) |
| Agent 2 | Phase 3-4 | Service, Controller, DTO | Agent 1 완료 후 |

**실행 순서**: Agent 1 → Agent 2 (의존 관계) 또는 Agent 1 ∥ Agent 2 (독립)
```
